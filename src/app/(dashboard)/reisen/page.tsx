    ...
    <div>
      ...
      {trips.length === 0 ? (
        <div style={{ background:"#fff", borderRadius:12, padding:"48px 24px", border:"1px solid #d4d0c8", textAlign:"center" }}>
          ...
          <div style={{ fontSize:14, color:"#9e9a92", marginTop:6, marginBottom:16 }}>Erstelle deine erste Reise!</div>
          <Link href="/reisen/neu" style={{ padding:"10px 20px", borderRadius:8, background:"#8b0a1e", color:"#fff", textDecoration:"none", fontWeight:700 }}>+ Erste Reise anlegen</Link>
        </div>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {trips.map((t: any) => {
            ...
            return (
              <Link key={t.id} href={`/reisen/${t.id}`} style={{ background:"#fff", borderRadius:12, padding:"16px 20px", border:"1px solid #d4d0c8", display:"flex", alignItems:"center", gap:16, textDecoration:"none", color:"inherit" }}>
                <div style={{ width:40, height:40, borderRadius:8, background:"#00305610", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>
                  {t.travelMode==="PRIVAT_PKW"?"🚗":"🚂"}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:700, fontSize:15, color:"#1a1815", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{t.purpose}</div>
