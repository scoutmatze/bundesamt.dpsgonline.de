...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rkInput = { profile: { lastName: user.lastName||"", firstName: [user.title, user.firstName].filter(Boolean).join(" ")||"", street: user.street||"", zip: user.zipCode||"", city: user.city||"", accountHolder: user.accountHolder||`${user.firstName||""} ${user.lastName||""}`.trim(), bank: user.bank||"", iban: await (async()=>{let v=user.ibanEncrypted||"";try{const{decrypt}=await import("@/lib/encryption");return decrypt(v)}catch{return v}})(), bic: user.bic||"", signaturePath: sigPath }, trip: { purpose: trip.purpose, route: (()=>{ if(trip.kmLegs){try{const legs=JSON.parse(trip.kmLegs);if(legs.length>0)return legs.map((l:any)=>l.from).concat(legs[legs.length-1].to).join(" – ")}catch{}} return trip.route||"" })(), startDate: fmtDate(trip.startDate), startTime: trip.startTime||"", endDate: fmtDate(trip.endDate||trip.startDate), endTime: trip.endTime||"", mode: trip.travelMode, pkwReason: trip.pkwReason||"", licensePlate: trip.licensePlate||"", km: trip.travelMode==="PRIVAT_PKW"?(trip.kmTotal||0):0 }, costs: { travel: fmt(trip.travelMode==="PRIVAT_PKW"?(trip.kmAmount||0):byC("FAHRT")), kmMoney: fmt(trip.travelMode==="PRIVAT_PKW"?(trip.kmAmount||0):0), lodging: fmt(byC("UNTERKUNFT")), meals: fmt(byC("VERPFLEGUNG")), other: fmt(byC("NEBENKOSTEN")), subtotal: fmt(trip.travelMode==="PRIVAT_PKW"?(trip.kmAmount||0)+byC("UNTERKUNFT")+byC("VERPFLEGUNG")+byC("NEBENKOSTEN"):total), reimbursement: fmt(0), total: fmt(trip.travelMode==="PRIVAT_PKW"?(trip.kmAmount||0)+byC("UNTERKUNFT")+byC("VERPFLEGUNG")+byC("NEBENKOSTEN"):total) }, checkboxes: { bankKnown: true, bahn: trip.travelMode==="BAHN", auto: trip.travelMode==="PRIVAT_PKW", dienstwagen: trip.travelMode==="DIENSTWAGEN"||trip.travelMode==="MIETWAGEN", flugzeug: trip.travelMode==="FLUGZEUG", schiff: false, co2: trip.co2Offset||false } };
