...
async function findOrCreateTrip(userId, ticketDate, subject) {
  // Find a DRAFT trip where the ticket date falls within the trip's date range (±1 day)
  const trips = await pool.query(
    `SELECT * FROM "Trip" WHERE "userId" = $1 AND status = 'DRAFT' ORDER BY "startDate" ASC`,
    [userId]
  );
  ...
  for (const trip of trips.rows) {
    ...
    const dayBefore = new Date(tripStart); dayBefore.setDate(dayBefore.getDate() - 1);
    const dayAfter = new Date(tripEnd); dayAfter.setDate(dayAfter.getDate() + 1);

    if (ticketDate >= dayBefore && ticketDate <= dayAfter) {
      // Extend trip dates if needed
      if (ticketDate < tripStart) {
        await pool.query('UPDATE "Trip" SET "startDate" = $1, "updatedAt" = NOW() WHERE id = $2', [ticketDate, trip.id]);
      }
      if (ticketDate > tripEnd) {
        await pool.query('UPDATE "Trip" SET "endDate" = $1, "updatedAt" = NOW() WHERE id = $2', [ticketDate, trip.id]);
      }
      return trip.id;
    }
  }

  // No matching trip → create new one
  const tripId = "c" + randomBytes(12).toString("hex");
  await pool.query(
    'INSERT INTO "Trip" (id, "userId", purpose, "startDate", "travelMode", status, "createdAt", "updatedAt") VALUES ($1,$2,$3,$4,\'BAHN\',\'DRAFT\',NOW(),NOW())',
    [tripId, userId, subject || "Neue Reise", ticketDate]
  );
  console.log("  + Neue Reise erstellt: " + (subject || "Neue Reise"));
  return tripId;
}
        ...
        try{
          ...
          for(const att of atts){
            ...
            if(/\.pdf$/i.test(safe)){
              ...
              if(t&&!t.error){
                if(t.amount)amt=t.amount;
                if(t.from)fromS=t.from;
                if(t.to)toS=t.to;
                if(t.is_handyticket)ht=true;
                if(t.order_nr)orderNr=t.order_nr;
                if(t.date){const[d,m,y]=t.date.split(".");dt=new Date(`${y}-${m}-${d}`)}
                console.log(`  Parsed: ${amt}€ ${fromS||"?"} → ${toS||"?"} ${ht?"(HT)":""} #${orderNr||"?"}`);
              }
            }

            // Smart grouping: find trip by date or create new
            const tripId = await findOrCreateTrip(u.id, dt, parsed.subject || "Reise");

            const rid="c"+randomBytes(12).toString("hex");
            const desc = orderNr
              ? `DB #${orderNr}: ${fromS||"?"} → ${toS||"?"}`
              : `Per E-Mail: ${parsed.subject||att.filename}`;
            await pool.query(
              'INSERT INTO "Receipt"(id,"tripId",description,amount,date,category,"isHandyticket","fromStation","toStation","fileName","filePath","mimeType","fileSize","createdAt")VALUES($1,$2,$3,$4,$5,\'FAHRT\',$6,$7,$8,$9,$10,$11,$12,NOW())',
              [rid,tripId,desc,amt,dt,ht,fromS,toS,safe,fp,att.contentType,att.size]
            );
            ...
            if(fromS&&toS){
              ...
              if(rr.rows.length>0){
                const route=rr.rows.map(r=>r.fromStation).concat(rr.rows[rr.rows.length-1].toStation).join(" – ");
                await pool.query('UPDATE "Trip" SET route=$1,"updatedAt"=NOW() WHERE id=$2',[route,tripId]);
              }
            }
